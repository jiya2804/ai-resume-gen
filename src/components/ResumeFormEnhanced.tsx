...
interface ResumeFormEnhancedProps {
  data: ResumeData;
  onChange: (data: ResumeData) => void;
}

export const ResumeFormEnhanced = ({ data, onChange }: ResumeFormEnhancedProps) => {
  ...
  const enhanceExperienceDescription = (id: string, index: number) => {
    const experience = data.experience.find((exp) => exp.id === id);
    if (!experience) return;

    const originalText = experience.description[index];
    const enhanced = enhanceText(originalText, 'experience');

    updateExperienceDescription(id, index, enhanced);
  ...
  const enhanceSkills = () => {
    ...
    const enhanced = enhanceText(skillsText, 'skills');
    updateSkills(enhanced);
    
    toast({
      title: "✨ AI Enhanced",
      description: "Your skills have been enhanced with proficiency levels!",
    });
  };
                ...
                <div>
                  ...
                  {exp.description.map((desc, idx) => (
                    <div key={idx} className="mb-2 flex gap-2">
                      ...
                      />
                      <Button
                        onClick={() => enhanceExperienceDescription(exp.id, idx)}
                        size="sm"
                        variant="outline"
                        className="shrink-0"
                      >
